...
(function (global) {
    ...
    var img_koma=null;img_koma = new Image();img_koma.src = "koma.png";
    var storage=null;    
    try{
    ...
    function init(){
        ...
        canv_koma =document.createElement("canvas");
        canv_koma.width=ctx.canvas.width;
        canv_koma.height=ctx.canvas.height;
        ...
        //画像読み込み成功時
        img_koma.onload = function() {
            img_bk_loaded=true;
            flush(true,false);
        }
        //画像読み込み失敗時
        img_koma.onerror = function() { 
            flush(true,false);
        } 
        //もう既に読み込み終わってた時
        if(img_koma.width!=0){
            img_koma_loaded=true;
            flush(true,false);
        }
        // 2.5秒後に強制描画※Googleの検索結果から飛ぶとなぜか描画が走らない事があるので。
        setTimeout(function() {
     ...
     */
    function drawKoma(){
        var ctx_koma=canv_koma.getContext('2d');
        if(img_koma_loaded){
            ctx_koma.drawImage(img_koma,0,0, ctx.canvas.width, ctx.canvas.height,0,0,500,500);
        }
        return canv_koma;
    }
    /** 
    ...
    function drawCover(){
       ...
       var message="Play"
        ...
        //文字２
        message="えるえるえるね"
        fontsize=Math.round(cellSize*0.7);
        ctx_cover.font = "bold "+fontsize+"px sans-serif";
        ctx_cover.fillStyle = COLOR_WHITE;
        ctx_cover.beginPath();
    ...
    function drawPiece(wkCtx,x,y,number,goal){
        ...
        if(img_koma_loaded){
            ...
            wkCtx.drawImage(drawKoma(true),x+cellSize/10,y+cellSize/10,cellSize-1*cellSize/5,cellSize-1*cellSize/5);
